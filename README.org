...
** Old stuff
   ...
   What doc-em does is basically this:
   - Adds a function ~doc-em--set-timer-if-moved~ in buffer's local
     ~post-command-hook~ which starts or restarts a timer. This hook is
     used as all moving executes a command, including mouse clicks.
   - The timer, then, checks whether the point has moved from where it
     was earlier in the buffer. If it has, it will try and search for a
     matching anchor. First try if there's an anchor at current
     ~point~, and if not, then start going up the scope in a loop until
     anchor is found.
   - Searching for anchor requires detecting the program-being-edited's
     scopes in the buffer. Uses standard Emacs' function
     ~backward-up-list~ for this.
   - Scopes are walked upwards until a comment with an anchor is found
     (either at point of opening scope, or one line down). Uses the
     =newcomment.el= standard package.
   - Parsing an anchor with regexps.
   - Opening a window with the documentation in it, or showing an
     already opened window with the document.
   - Jumping to the correct place in the document.

    (How to find the anchor?)

    (How about moving at the anchor directly?)

** Scope detection

** Handling comments

   The standard Emacs package =newcomment.el= is used for handling the
   comments.

*** Comment line parsing

    The blabla will connect to the xyzzy.


** Jumping in document

   Currently, only org-mode documents are supported. To expand to
   other modes, the function ~doc-em--org-jump-to-corre~

   The location is searched with ~org-goto-local-search-headings~
   function.
